...
// ChoisirCoupRobot renvoie un indice de colonne valide pour l'IA (ou -1 si aucun)
func (j *Jeu) ChoisirCoupRobot() int {
	j.mu.Lock()
	defer j.mu.Unlock()

	// Collecter colonnes valides
	cols := []int{}
	for c := 0; c < Colonnes; c++ {
		if j.Plateau[0][c] == Vide {
			cols = append(cols, c)
		}
	}
	if len(cols) == 0 {
